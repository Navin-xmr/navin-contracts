...
#[test]
fn test_remove_carrier_from_whitelist_writes_audit_entry() {
    let (env, client, admin) = setup_test_env();
    let carrier = Address::generate(&env);
    client.add_carrier(&admin, &carrier);
    client.add_carrier_to_whitelist(&admin, &carrier);

    client.remove_carrier_from_whitelist(&admin, &carrier);

    let entries = client.query_audit_history_for_target(&carrier);
    // add_carrier (0) + add_carrier_to_whitelist (1) + remove_carrier_from_whitelist (2)
    assert_eq!(entries.len(), 3);
    let entry = entries.get(2).unwrap();
    assert_eq!(entry.event_type, AuditEventType::CarrierUnwhitelisted);
    assert_eq!(entry.actor, admin);
    assert_eq!(entry.target, carrier);
}

// ── Carrier suspension (Issue #771) ──────────────────────────────────────

#[test]
fn test_suspend_carrier_writes_audit_entry() {
    let (env, client, admin) = setup_test_env();
    let carrier = Address::generate(&env);
    client.add_carrier(&admin, &carrier);
    ...
    assert_eq!(entries.len(), 2);
    let entry = entries.get(1).unwrap();
    assert_eq!(entry.event_type, AuditEventType::CarrierSuspended);
    assert_eq!(entry.actor, admin);
    assert_eq!(entry.target, carrier);
}

#[test]
fn test_reactivate_carrier_writes_audit_entry() {
    let (env, client, admin) = setup_test_env();
    let carrier = Address::generate(&env);
    client.add_carrier(&admin, &carrier);
    client.suspend_carrier(&admin, &carrier);
    ...
    assert_eq!(entries.len(), 3);
    let entry = entries.get(2).unwrap();
    assert_eq!(entry.event_type, AuditEventType::CarrierReactivated);
    assert_eq!(entry.actor, admin);
    assert_eq!(entry.target, carrier);
}

// ── Company suspension (Issue #772) ──────────────────────────────────────

#[test]
fn test_suspend_company_writes_audit_entry() {
    let (env, client, admin) = setup_test_env();
    let company = Address::generate(&env);
    client.add_company(&admin, &company);
    ...
    assert_eq!(entries.len(), 2);
    let entry = entries.get(1).unwrap();
    assert_eq!(entry.event_type, AuditEventType::CompanySuspended);
    assert_eq!(entry.actor, admin);
    assert_eq!(entry.target, company);
}

#[test]
fn test_reactivate_company_writes_audit_entry() {
    let (env, client, admin) = setup_test_env();
    let company = Address::generate(&env);
    client.add_company(&admin, &company);
    client.suspend_company(&admin, &company);
    ...
    assert_eq!(entries.len(), 3);
    let entry = entries.get(2).unwrap();
    assert_eq!(entry.event_type, AuditEventType::CompanyReactivated);
    assert_eq!(entry.actor, admin);
    assert_eq!(entry.target, company);
}
...
fn test_cleanup_audit_logs_rejects_non_admin() {
    ...
    let result = client.try_cleanup_audit_logs(&intruder, &(env.ledger().timestamp() + 1));
