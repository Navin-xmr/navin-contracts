...
use crate::{
    test_utils::{setup_env, advance_ledger_time},
    types::ShipmentStatus,
    NavinShipment, NavinShipmentClient,
};
use soroban_sdk::{contract, contractimpl, testutils::Address as _, Address, Env, BytesN, Vec};
...
fn test_detect_anomalies_and_escrow() {
    ...
    let id1 = client.create_shipment(&company, &receiver, &carrier, &data_hash1, &Vec::new(&env), &deadline);
    advance_ledger_time(&env, 1);
    let id2 = client.create_shipment(&company, &receiver, &carrier, &data_hash2, &Vec::new(&env), &deadline);
