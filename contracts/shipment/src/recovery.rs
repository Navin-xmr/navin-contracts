...
fn is_valid_recovery_transition(from: &ShipmentStatus, to: &ShipmentStatus) -> bool {
    ...
    match (from, to) {
        ...
        (_, Cancelled) => true,
        // Allow transition to Disputed from non-terminal states, including partial states
        (Created | InTransit | AtCheckpoint | PartiallyDelivered | PartiallyRefunded, Disputed) => true,
        // Allow transition to Delivered from Disputed, InTransit, AtCheckpoint, and partial states
        (Disputed | InTransit | AtCheckpoint | PartiallyDelivered | PartiallyRefunded, Delivered) => true,
        // Recovery paths out of PartiallyDelivered
        (PartiallyDelivered, InTransit) => true,
        (PartiallyDelivered, AtCheckpoint) => true,
        (PartiallyDelivered, PartiallyDelivered) => true,
        (PartiallyDelivered, PartiallyRefunded) => true,
        // Recovery paths out of PartiallyRefunded
        (PartiallyRefunded, InTransit) => true,
        (PartiallyRefunded, AtCheckpoint) => true,
