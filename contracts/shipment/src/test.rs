...
fn test_revoke_role_then_create_shipment_fails() {
    ...
    let _id = client.create_shipment(&company, &receiver, &carrier,
    ...
    let result =
        client.try_create_shipment(&company, &receiver, &carrier,
        &token_contract, &hash, &milestones, &deadline);
    assert!(result.is_err());
}
...
// ─── Issue #142: Per-shipment token address tests ─────────────────────────────

#[test]
fn test_single_shipment_single_token() {
    let (env, client, admin, token_contract) = setup_shipment_env();
    let company = Address::generate(&env);
    let receiver = Address::generate(&env);
    let carrier = Address::generate(&env);
    ...
    let stored_token = client.get_shipment_token(&shipment_id);
    assert_eq!(stored_token, token_contract);

    let shipment = client.get_shipment(&shipment_id);
    assert_eq!(shipment.token_address, token_contract);
}

#[test]
fn test_two_shipments_two_different_tokens_independent_balances() {
    let (env, client, admin, token_a) = setup_shipment_env();
    ...
    // Create shipment 1 with token_a
    let id1 = client.create_shipment(
        &company,
        &receiver,
        &carrier,
    ...
    super::test_utils::advance_ledger_time(&env, 400);

    // Create shipment 2 with token_b
    let id2 = client.create_shipment(
        &company,
        &receiver,
        &carrier,
    ...
    // Each shipment stores its own token
    assert_eq!(client.get_shipment_token(&id1), token_a);
    assert_eq!(client.get_shipment_token(&id2), token_b);

    // Tokens are different
    assert_ne!(token_a, token_b);

    // Deposit into shipment 1 uses token_a
    client.deposit_escrow(&company, &id1, &1000);
    assert_eq!(client.get_shipment(&id1).escrow_amount, 1000);
    // Shipment 2 is unaffected
    assert_eq!(client.get_shipment(&id2).escrow_amount, 0);
}

#[test]
fn test_cross_token_rejection_wrong_token_deposit_fails() {
    // Create shipment with a failing token to verify that the token stored
    // per shipment is used (not the global one), causing a transfer failure
    let (env, client, admin, _global_token) = setup_shipment_env_with_failing_token();
    let company = Address::generate(&env);
    let receiver = Address::generate(&env);
    let carrier = Address::generate(&env);
    let data_hash = BytesN::from_array(&env, &[3u8; 32]);
    ...
    // Use the failing token as the per-shipment token
    let failing_token = env.register(failing_token::FailingMockToken {}, ());
    client.initialize(&admin, &_global_token);
    client.add_company(&admin, &company);
    ...
    // deposit_escrow should fail because the per-shipment token transfer fails
    let result = client.try_deposit_escrow(&company, &shipment_id, &500);
    assert_eq!(result, Err(Ok(crate::NavinError::TokenTransferFailed)));
}

#[test]
fn test_get_shipment_token_returns_correct_address_per_shipment() {
    let (env, client, admin, token_a) = setup_shipment_env();
    let token_b = env.register(MockToken {}, ());
    ...
    assert_eq!(client.get_shipment_token(&id1), token_a);
    assert_eq!(client.get_shipment_token(&id2), token_b);
    assert_ne!(client.get_shipment_token(&id1), client.get_shipment_token(&id2));
}
